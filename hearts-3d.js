          ...
          (function () {
            ...
            e.load(
              ...
              function (e) {
                ...
                u = new h.InstancedMesh(l, r, 60); // Aumentei um pouco a quantidade
        ...
        if (mesh.isInstancedMesh) {
          ...
          for (let j = 0; j < bodies.length; j++) {
            ...
            if (origin.y() < -3) {
              x.setValue(0, 0, 0);
              M.setIdentity();
              // Reposiciona aleatoriamente acima
              M.setOrigin(new g.btVector3((Math.random()-0.5)*1.5, 2 + Math.random(), (Math.random()-0.5)*0.5));
              body.setWorldTransform(M);
              body.setLinearVelocity(x);
              body.setAngularVelocity(x);
              body.clearForces();
            } else {
              // Atualiza matriz visual
              const origin = M.getOrigin();
              const rotation = M.getRotation();
              const compose = new h.Matrix4().compose(
                new h.Vector3(origin.x(), origin.y(), origin.z()),
                new h.Quaternion(rotation.x(), rotation.y(), rotation.z(), rotation.w()),
                new h.Vector3(1, 1, 1) // Escala fixa por enquanto, pois está no geometry
              );
              
              // Copia para o array da instância
